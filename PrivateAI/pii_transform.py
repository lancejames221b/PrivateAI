try:
    ...
    if os.environ.get('DISABLE_TRANSFORMERS', 'false').lower() == 'true':
        ...
        import logging
        logging.warning("transformers module disabled by environment variable.")
    else:
    ...
    TRANSFORMERS_AVAILABLE = False
    import logging
    logging.warning("transformers module not available. Using regex-only transformations.")
...
try:
    ...
    import logging
    logging.warning("codename_generator module not available. Using basic placeholder generation.")
try:
    ...
    try:
        ...
        logging.warning("Custom TransformersRecognizer not available. Using standard Presidio recognizers.")
...
def get_db_connection_pii():
    """Returns the global connection for pii_transform module"""
    global conn, cursor
    # This might need locking if used concurrently, but mitmproxy addons are usually single-threaded
    try:
...
def get_replacement(word, entity_type=None):
    """Get existing replacement or create new one with encryption"""
    try:
        current_conn = get_db_connection_pii()
        if not current_conn:
            logger.error("No database connection available in get_replacement")
            return f"__DB_ERROR_{str(uuid.uuid4())[:8]}__"
            
        current_cursor = current_conn.cursor()
        # Check if the mapping exists
        current_cursor.execute("SELECT replacement, is_encrypted FROM mappings WHERE original = ? OR original = ?", 
        ...
        if result:
            ...
            current_cursor.execute("UPDATE mappings SET last_used = ? WHERE original = ? OR original = ?", 
            ...
            current_cursor.execute("INSERT INTO mappings VALUES (?, ?, ?, ?, ?, ?)",
        ...
        print(f"Error in get_replacement: {str(e)}")
        # Return a fallback replacement in case of error
        return f"__ERROR_{str(uuid.uuid4())[:8]}__"
