...
# Initialize logger
logger = get_logger("ai-security-proxy", "logs/aiproxy.log", log_level_name)
...
class ThreadingHTTPServer(ThreadingMixIn, TCPServer):
    ...
    def handle_error(self, request, client_address):
        ...
        try:
            ...
            logger.error(f"Error processing request from {client_address}: {exc_type.__name__}: {exc_value}")
            
            # Log the traceback
            tb_lines = traceback.format_exception(exc_type, exc_value, exc_traceback)
            logger.error("Traceback details:\n" + "".join(tb_lines))
            
            # Log additional request information if available
            if hasattr(request, 'requestline'):
                logger.error(f"Request line: {request.requestline}")
            if hasattr(request, 'path'):
                logger.error(f"Request path: {request.path}")
                
        except Exception as logging_error:
            # If we can't log the error properly, at least try to log something
            logger.critical(f"Error while logging error: {logging_error}")
